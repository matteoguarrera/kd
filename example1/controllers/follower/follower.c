...
double process_camera_image(const unsigned char *image) {
  ...
  const unsigned char REF[3] = {0, 0, 0};    // road yellow (BGR format)
...
int main(int argc, char **argv) {
  ...
  while (wbu_driver_step() != -1) {
    ...
    if (i % (int)(TIME_STEP / wb_robot_get_basic_time_step()) == 0) {
      ...
      int number_of_recognition_objects = wb_camera_recognition_get_number_of_objects(camera);
      const WbCameraRecognitionObject *recognition_objects = wb_camera_recognition_get_objects(camera);
      double distance_to_lead = UNKNOWN;
      for (int k = 0; k < number_of_recognition_objects; k++) {
        if (strcmp(recognition_objects[k].model, "BMW X5") == 0) {
          double x = recognition_objects[k].position[0];
          double y = recognition_objects[k].position[1];
          double z = recognition_objects[k].position[2];
          distance_to_lead = sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2));
          break;
        }
      }
