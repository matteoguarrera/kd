...
# SpacePoint(objects=StructPoint(object0=StructPoint(angularSpeed=0.0, controller=None, elevation=None, heading=-0.048992500761284245, lastActions=None, length=1.0, position=(-65.92858860953253, -105.1627133023631), positionOffset=(0.0, 0.0), resetController=True, rotationOffset=0.0, speed=0.0, type='Follower', velocity=(0.0, 0.0), webotsName='FOLLOWER', webotsObject=None, webotsType=None, width=1.0),
# object1=StructPoint(angularSpeed=0.0, color=[0.0, 0.0, 1.0],
# controller=None, elevation=None, heading=0.0, lastActions=None,
# length=1.0, position=(53.1530612244898, -11.712121212121211), positionOffset=(0.0, 0.0),
# resetController=True, rotationOffset=0.0, speed=0.0, type='ObstacleL', velocity=(0.0, 0.0),
# webotsName='OBSTACLE', webotsObject=None, webotsType=None, width=1.0),
# object2=StructPoint(angularSpeed=0.0, color=[0.0, 0.0, 0.0], controller=None,
# elevation=None, heading=0.0, lastActions=None, length=1.0, position=(36.11224489795919, -25.5),
# positionOffset=(0.0, 0.0), resetController=True, rotationOffset=0.0, speed=0.0, type='ObstacleS',
# velocity=(0.0, 0.0), webotsName='OBSTACLE', webotsObject=None, webotsType=None, width=1.0),
# object3=StructPoint(angularSpeed=0.0, color=[0.0, 0.0, 0.0],
# controller=None, elevation=None, heading=0.0, lastActions=None, length=1.0,
# position=(63.868686868686865, -35.80612244897959), positionOffset=(0.0, 0.0),
# resetController=True, rotationOffset=0.0, speed=0.0, type='ObstacleR', velocity=(0.0, 0.0),
# webotsName='OBSTACLE', webotsObject=None, webotsType=None, width=1.0),
# object4=StructPoint(angularSpeed=0.0, controller=None, elevation=None, heading=0.0, ind='S',
# lastActions=None, length=1.0, mode='falsify', position=(0, 0), positionOffset=(0.0, 0.0),
# resetController=True, rotationOffset=0.0, speed=0.0, type='Scenario', velocity=(0.0, 0.0),
# webotsName='SCENARIO', webotsObject=None, webotsType=None, width=1.0),
# object5=StructPoint(angularSpeed=0.0, controller=None, elevation=None,
# heading=-0.08591044646951464, lastActions=None, length=1.0, position=(-54.827927119285114, -105.98074563020639),
# positionOffset=(0.0, 0.0), resetController=True, rotationOffset=0.0, speed=0.0, type='Lead', velocity=(0.0, 0.0),
# webotsName='LEAD', webotsObject=None, webotsType=None, width=1.0)), params=StructPoint())
...
cmd_str = "cd ~/Documents/kd/example1/controllers/lead/build &&  cmake .. && make"  # cmake .. &&
subprocess.run(cmd_str, shell=True)
...
class Task(webots_task):
        ...
        for obj in sample.objects:
            if obj.webotsName == "SUBSCENARIO":
                ...
                if controller_arg == "" or controller_arg not in obj.ind:
                    ...
                    # print('reset straight')
                    # lead.getField("controllerArgs").setMFString(0, "S")
                    ego.restartController()
                    lead.restartController()
            if obj.webotsName == "SCENARIO":
                # self.ind = obj.ind
                # self.ind = "R"
    ...
    def run_task(self, sample):
        ...
        lead_x, lead_y, _ = lead.getPosition()
        sim_steps = 0
        print(f'SUBSCENARIO: {self.subscenario}\t IND: {self.ind}')
        ...
        print(f'EGO: {ego_x:2.2f} {ego_y:2.2f}\t\tLEAD: {lead_x:2.2f} {lead_y:2.2f}')
        ...
        if self.trajectory[-1] >= 0.0:
            ...
            try:
                file_ = self.mode + "_csvs/" + self.subscenario + "_post_conditions.csv"
                print(file_)
                post_conditions = np.genfromtxt(file_,
            ...
            if delta_sem is not None and np.all(delta_sem <= self.epsilon):
                ...
                return {}  # This causes a ValueError that is caught by the analyze script.
        else:
            if self.mode == "falsify":
                print("Falsification ended successfully.")
                sys.exit()  # This causes a EOFError that is caught by the analyze script.
            else:
                pass
        sim_results = {}
